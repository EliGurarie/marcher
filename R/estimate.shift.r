...
#' 
#' @aliases estimate.mouf.nls, getP.nls, geMu.nls, getTau
#' @example ./examples/estimate.shift.example.r


estimate.shift <- function(T, X, Y, n.clust = 2,  
                           ...
                           area.direct = NULL)
{
  ...
  if(n.clust == 2) FUN <- getMu else if(n.clust > 2) FUN <- getMu.multi
...
print.shiftfit <- function(fit){
  with(fit, {
   ...
   if(!rangemodel.given) 
    ...
    if(!("p.CI" %in% names(fit))) print(t(p.hat)) else print(p.CI)
    cat(paste0("\nlog-likelihood: ", signif(ll,4), "; AIC: ", signif(aic,4), "; degrees of freedom: ", signif(df,4), "\n"))
  })
  invisible(fit)
}

summary.shiftfit <- function(fit){print.shiftfit(fit); invisible(fit)}

AIC.shiftfit <- function(fit) fit$aic
logLik.shiftfit <- function(fit) fit$ll
