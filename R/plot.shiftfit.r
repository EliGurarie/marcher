...
#' 
#' @param FIT a fitted range shift object, i.e. output of the \code{\link{estimate.shift}}
...
#' @example ./examples/estimate.shift.example.r
	
plot.shiftfit <- function(FIT, ns = c(n.sims = 1e3, n.times = 1e2, n.bins = 10), 
	...
	p.CI[ts,]$CI.low <- pmax(p.CI[ts,]$CI.low, min(FIT$T))
	...
	if(n.clust == 2){
		...
		# draw bells
		with(p.mu.hat, {
				polygon.circle(x1, y1, r95, col=g1, bor=NA)
				polygon.circle(x2, y2, r95, col=g1, bor=NA)
				polygon.circle(x1, y1, r50, col=g2, bor=NA)
				polygon.circle(x2, y2, r50, col=g2, bor=NA)})
	...
	if(n.clust == 3){
		...
		with(p.mu.hat, {
				polygon.circle(x1, y1, r95, col=g1, bor=NA)
				polygon.circle(x2, y2, r95, col=g1, bor=NA)
				polygon.circle(x3, y3, r95, col=g1, bor=NA)}
				)
		...
		with(p.mu.hat, {
				polygon.circle(x1, y1, r50, col=g2, bor=NA)
				polygon.circle(x2, y2, r50, col=g2, bor=NA)
				polygon.circle(x3, y3, r50, col=g2, bor=NA)
	...
	####################
	FIT$p.CI <- p.CI
	if(plot.ts)		plotFit.ts(FIT, CI.cols, pt.cex = pt.cex)
}
...
plotFit.ts <- function(FIT, pt.col = "antiquewhite", CI.cols = NULL, n.sims = 1e3, xlab = "Time", pt.cex = 1, ...){
  ...
  gr1 <- grey(.2)
	...
	if(is.null(CI.cols)){
			...
			g2 <- blues[2] #g2 <- rgb(0.5,0.5,.8)
			g3 <- blues[3] #g3 <- rgb(0.05,0.05, 0.4)
		} else {g1 <- cols[1]; g2 <- cols[2]; g3 <- cols[3]}
	...
	if(n.clust == 3)  GETMU <- getMu.multi else GETMU <- getMu
...
getBars <- function(p.mu.hat, p.mu.se, n.sims = 1e3, n.time = 1e2, n.bins = 25, stretch = 0){	
  ...
  Y.hat <- getMu(T.sim, p.mu.hat)[,2]
	...
	M.rotate <- complex(arg = -theta, mod = 1)
	Zr <- Z.sim * M.rotate
...
polygon.circle <- function(x, y, r, n=100, ...){
  z.circle <- complex(arg = seq(0, 2*pi, length=n), mod = r) + x + 1i*y
  polygon(Re(z.circle), Im(z.circle), ...)
}
polygon.XY <- function(X1, X2, Y1, Y2, ...)
  polygon(c(X1, X2[length(X2):1]), c(Y1, Y2[length(Y2):1]), bor=NA, ...)
polygon.Z <- function(Z1, Z2, ...)
  polygon(c(Re(Z1), Re(Z2[length(Z2):1])), c(Im(Z1), Im(Z2[length(Z2):1])), bor=NA, ...)
polygon.CI <- function(T, low, high, ...)
  polygon(c(T, T[length(T):1]), c(low, high[length(high):1]), bor=NA, ...)
