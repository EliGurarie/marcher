...
#' @param time time (can be a \code{\link{POSIX}})
#' @param {x,y} Coordinates.  Can be two separate vectors OR a complex "x" OR a two-column matrix/date-frame. 
...
#' with(Michela, scan.track(time = time, x = x, y = y, main="Michela"))
#' 
#' ## Simulated track
#' 
#' time <- 1:200
#' Mean <- getMu(T = time, p.m = c(x1 = 0, y1 = 0, x2 = 10, y2 = 10, t1 = 90, dt = 20))
#' SimTrack <- simulate.shift(T = time, tau = c(tau.z = 5), mu = Mean, A = 40)
...
scan.track <- function(track=NULL, time, x, y=NULL, layout = NULL, auto.par = NULL, ...)
{
